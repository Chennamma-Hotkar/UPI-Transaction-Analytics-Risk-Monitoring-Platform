...
.tf-btn{{font-size:10px;padding:3px 10px;border-radius:5px;border:0.5px solid var(--border);background:transparent;color:var(--muted);cursor:pointer;transition:all .15s;}}
.tf-btn.active{{background:var(--border);color:var(--txt);border-color:var(--blue);}}
.tf-btn:hover:not(.active){{color:var(--txt);border-color:var(--muted);}}
.exp-btn{{font-size:10px;padding:3px 10px;border-radius:5px;border:0.5px solid var(--blue);background:rgba(55,138,221,.1);color:var(--blue);cursor:pointer;transition:all .15s;}}
.exp-btn:hover{{background:rgba(55,138,221,.2);}}
</style>
</head>
<body>
...
<div class="hdr">
  ...
  <div style="display:flex;align-items:center;gap:14px;">
    ...
    <div style="display:flex;align-items:center;gap:4px;">
      <button class="tf-btn active" onclick="setTimeFilter('24h',this)">24h</button>
      <button class="tf-btn" onclick="setTimeFilter('7d',this)">7d</button>
    </div>
...
const TIME_MULT={{'24h':1,'7d':7}};
const TIME_LABEL={{'24h':'today','7d':'last 7 days'}};
...
function exportCSV(){{
  const hdr=['Transaction ID','Amount (INR)','Category','Bank','Timestamp','Risk Score','Fraud Flag'].join(',');
  const rows=txFeed.map(t=>[t.id,Math.round(t.amt),t.cat,t.bk,t.ts,Math.round(t.risk),t.fraud?'FRAUD':'CLEAN'].join(','));
  const csv=[hdr,...rows].join('\n');
  const blob=new Blob([csv],{{type:'text/csv'}});
  const a=document.createElement('a');
  a.href=URL.createObjectURL(blob);
  a.download='upi_transactions_'+new Date().toISOString().slice(0,10)+'.csv';
...
function update(){{
  ...
  const mult=TIME_MULT[activeTimeFilter];const tlbl=TIME_LABEL[activeTimeFilter];
  document.getElementById('kv-vol').textContent=fVol(totalVol*mult);
  document.getElementById('ks-vol').textContent=fNum(totalTx*mult)+' transactions '+tlbl;
  ...
  document.getElementById('kv-fr').textContent=fNum(fraudAlerts*mult);
  document.getElementById('ks-fr').textContent=Math.round(fraudAlerts*mult*.11)+' high risk '+tlbl;
